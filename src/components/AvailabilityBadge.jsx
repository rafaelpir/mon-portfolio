...
export default function AvailabilityBadge({ availableDate, alternance, status, isDarkMode, textEffectsEnabled }) {
  return (
    <div className={`inline-flex flex-row items-center gap-6 md:gap-8 px-8 md:px-12 py-5 md:py-6 rounded-full transition-all duration-500 hover:scale-105 hover:shadow-2xl backdrop-blur-lg shadow-2xl border-4 ${
      isDarkMode
      ...
      {/* Pulsing dot avec glow effect - plus gros et plus visible */}
      <div className="relative flex items-center justify-center shrink-0">
        <span className={`absolute w-10 h-10 rounded-full animate-ping ${
          isDarkMode ? 'bg-orange-300' : 'bg-orange-500'
        }`} style={{ opacity: 0.6 }}></span>
        <span className={`absolute w-6 h-6 rounded-full blur-lg ${
      ...
      <div className="flex flex-row items-center gap-4 md:gap-6">
        <span className={`text-base md:text-xl font-bold tracking-wide uppercase whitespace-nowrap ${
